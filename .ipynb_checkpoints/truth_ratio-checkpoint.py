...
def compute_classification_truth_ratio(probs, labels):
    """
    Computes the ratio of P(true_label) / (P(true_label) + sum of P(wrong_labels)).
    
    Since probabilities sum to 1, this simplifies to P(true_label),
    but we keep the explicit form for clarity and possible extensions.
    """
    p_para = probs[np.arange(len(labels)), labels]
    
    # Sum of all incorrect label probabilities
    probs_copy = probs.copy()
    probs_copy[np.arange(len(labels)), labels] = 0.0
    p_wrong_sum = probs_copy.sum(axis=1)
    
    ratio = p_para / (p_para + p_wrong_sum + 1e-12)
    return ratio.mean()

def truth_ratio(mdl, data_loader):
    probs, labels, _ = compute_all_outputs(mdl, data_loader)
    tr = compute_classification_truth_ratio(probs, labels)
    return tr
